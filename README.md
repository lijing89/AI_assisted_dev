...
检查项目中指定目录下的资源文件，并检查它们是否在代码中被引用。旨在帮助开发者发现未使用的资源，减小包体积。

**功能：**
- 🔍 深度扫描：遍历资源目录收集文件，并与代码引用进行比对
- 🧠 智能匹配：支持静态文件名匹配和动态命名启发式检查（防止误删动态加载资源）
- 🛡️ 安全模式：只读不删，对疑似未使用的文件进行风险提示
- 📊 详细报告：输出未使用文件列表、大小统计及潜在体积节省

**查看详情：** [skills/check-unused-assets/SKILL.md](skills/check-unused-assets/SKILL.md)
...
将项目中散乱的 `print`、`debugPrint` 和 `log` 调用替换为标准化的 `LogUtil` 方法，统一日志格式和管理。

**功能：**
- 🧹 自动替换：批量扫描 `.dart` 文件，将原生日志调用替换为 `LogUtil`
- 🧠 语义识别：根据上下文（如 `catch` 块、关键字）自动选择 `error`、`warning`、`info` 或 `debug` 级别
- 🏷️ 标签保留：自动保留原有的 `name` 参数作为日志标签
- ✅ 自动引入：自动添加 `LogUtil` 的 import 语句
...
| 技能名称 | 说明 | 适用场景 |
|---------|------|---------|
| check-unused-assets | 资源引用检查器 | 扫描未使用的资源文件，优化包体积 |
